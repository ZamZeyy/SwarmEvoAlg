...
public class swarm {
    public static void main(String[] args) throws IOException {
        ...
        runScout(test_problem,training_problem, bounds, 10000, 400);
 ...
 //calculate test fitness of the swarm
    public static double[] calculateTestFitness(double[][] swarm, DemandPrediction test_problem) {
        double[] tFitness = new double[swarm.length];
        for (int i = 0; i < swarm.length; i++) {
            tFitness[i] = test_problem.evaluate(swarm[i]);
        }
        return tFitness;
    }
    ...
    public static double[] runPsoScout(DemandPrediction test_problem, DemandPrediction training_problem, double[][] bounds, int swarmSize, int maxIterations, double w, double c1, double c2, double[] gBest) {
        var r = new Random();
        double[][] swarm = initialiseSwarm(bounds, r, swarmSize);
        double[][] velocity = initialiseSwarm(bounds, r, swarmSize);
        double[][] pBest = initialiseSwarm(bounds, r, swarmSize);
        ...
        double[] tFitness = calculateFitness(swarm, test_problem);
        System.out.println("The best test fitness is2: " + tFitness[getBestParticle(tFitness)]);
    
        return gBest;
    }

      //scouting swarm to find a global best for the pso algorithm
      public static double[] runScout(DemandPrediction test_problem,DemandPrediction training_problem, double[][] bounds, int swarmSize, int maxIterations) {
        double w= 0.1;
        double c1= 2;
        double c2= -0.5;
        var r = new Random();
        ...
        runPsoScout( test_problem ,training_problem, bounds, 100000,  400, 0.1,  0.1,  0.1, gBest);
